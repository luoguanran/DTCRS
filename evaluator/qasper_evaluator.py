"""
Official script for evaluating models built for the Qasper dataset.
...
def get_answers_and_evidence(data, text_evidence_only):
    ...
    for paper_data in data:
        for i, question in enumerate(paper_data['qas']['question']):
            question_id = paper_data['qas']["question_id"][i]
            references = []
            for annotation_info in paper_data['qas']["answers"]:
                candidate_answers = annotation_info["answer"]
                for answer_info in candidate_answers:
                    if answer_info["unanswerable"]:
                        ...
                        if text_evidence_only:
                            # 忽略 figures 和 tables 的证据
                            evidence = [
                                text for text in answer_info["evidence"]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Test or dev set from the released dataset (in jsonl format)."
    )
    parser.add_argument(
        "--text_evidence_only",
        action="store_true",
    ...
    gold_data = []
    with open(args.gold, "r", encoding="utf8") as f:
        for line in f:
