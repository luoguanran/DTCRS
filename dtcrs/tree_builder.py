...
class TreeBuilder:
    ...
    def build_from_text(self, text: str, queries: List[str] = [], use_multithreading: bool = True, cluster_method: str="hierarchical") -> Tree:
        ...
        if queries:
            for index, query in enumerate(queries):
                __, node = self.create_node(index, query)
                query_nodes[index] = node
        ...
        print(f"预处理时间为 {elapsed_time:.4f} 秒")


        start_time = time.time()

        root_nodes = self.construct_tree(all_nodes, all_nodes, layer_to_nodes, query_nodes, cluster_method=cluster_method)
        # 结束计时
        end_time = time.time()

        # 计算执行时间（秒）
        elapsed_time = end_time - start_time
        print(f"聚类和摘要生成时间为 {elapsed_time:.4f} 秒")
