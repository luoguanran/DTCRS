...
from .FaissRetriever import FaissRetrieverConfig, FaissRetriever
...
class RetrievalAugmentationConfig:
    def __init__(
        ...
        self.embedding_model = embedding_model or SBertEmbeddingModel()
...
class RetrievalAugmentation:
    ...
    def add_documents_and_queries(self, docs, queries, cluster_method):
        """
        Adds documents to the tree and creates a TreeRetriever instance.

        Args:
            queries (list): The input query list to add to the tree.
        """
        if self.tree is not None:
            user_input = input(
                "Warning: Overwriting existing tree. Did you mean to call 'add_to_existing' instead? (y/n): "
        ...
        self.tree = self.tree_builder.build_from_text(text=docs, queries=queries, cluster_method=cluster_method)
        self.retriever = TreeRetriever(self.tree_retriever_config, self.tree)


    def add_faiss_documents(self, docs):
        """
        Adds documents to the tree and creates a TreeRetriever instance.
        ...
        self.retriever = FaissRetriever(FaissRetrieverConfig(embedding_model=self.embedding_model))
        self.retriever.build_from_text(docs)
    ...
    def answer_question(
        ...
        answer = self.qa_model.answer_question(context, question, answer_type)
